...
def get_data(infile, limit=2): # infile consists of lists of dictionaries
    ...
    for pt in data_oms:
        ...
        err = np.abs(DFT-FF)
        raw_err = FF-DFT
        if np.abs(DFT) < limit and np.abs(FF) < limit: # only consider points in the specific range
            if 'CO2' in n:
    ...
    return int_DFT_CO2, err_CO2, raw_err_CO2, int_FF_CO2, int_DFT_H2O, err_H2O, raw_err_H2O, int_FF_H2O
...
def binned_average(DFT_ads, pred_err, bins):
    ...
    for i,bin in enumerate(bins):
            ...
            bin_errs = []
            for DFT, pred in zip(DFT_ads, pred_err): #this is a horribly inefficient way to do this...
...
def bin_plot(ax, bins, heights, **kwargs): #stolen from https://stackoverflow.com/questions/36192074/manual-histogram-plot-in-python
    bins = list(bins)
    x1 = bins[:-1]
    x2 = bins[1:]
...
def get_Fig4a(raw_error_CO2, raw_error_H2O, b = 20, outfile = 'Fig5a.png'):
    ...
    plt.hist(raw_error_CO2, density = False, edgecolor = 'black', alpha = 0.5, bins = b, color = 'crimson', label = 'CO$_{2}$')
    plt.hist(raw_error_H2O, density = False, edgecolor = 'black', alpha = 0.5, bins = b, color = 'dodgerblue', label = 'H$_{2}$O')
    plt.xlabel('$E_{{int}}^{{FF}}$ – $E_{{int}}^{{DFT}}$ [eV]', fontsize = 20)
    plt.ylabel('Number of configurations', fontsize = 20)
    labels = ['<-1.00','-0.75','-0.50','-0.25','0.00','0.25','0.50','0.75','>1.00']
...
def get_Fig4c(DFT_CO2, err_CO2, outfile = 'Fig5c.png'):
    ...
    scatter = ax.scatter(x_sorted, y_sorted, c=z_sorted, cmap = custom_cmap, s=25, alpha=0.5)
...
if __name__ == '__main__':
    ...
    DFT_CO2, err_CO2, rerr_CO2, FF_CO2, DFT_H2O, err_H2O, rerr_H2O, FF_H2O = get_data(infile)
    ...
    print('Overall physisorption error: {} eV'.format(phys_err(DFT_CO2+DFT_H2O, FF_CO2+FF_H2O)))
    ...
    print('Overall chemisorption error: {} eV'.format(chem_err(DFT_CO2+DFT_H2O, FF_CO2+FF_H2O)))
